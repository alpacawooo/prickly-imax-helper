...
- [ ] Run a fresh 10-minute monitor soak with no 429, duplicate process, memory growth, login-profile loss, or checkout-guard failure. The first soak was invalidated on 2026-08-05 when a real seat match exposed a changed CGV mobile-ticket empty-state selector; it stopped before seat selection or payment. The 2026-08-05 16:35:02 KST baseline also failed safely after the dedicated CGV tab closed: the runtime stayed out of checkout with `match: null`, but recorded 21 `monitor_error` events and remained `recovering`; closed-tab/CDP recovery is now covered by regression tests before another baseline is started.
