...
- [x] Every eligible show enforces a Korea-time minimum lead of 180 minutes; legacy configurations without the field resolve to 180, CGV 24-29 hour clocks roll into the next calendar day, and setup permits only 180-1440 minutes.
...
- [x] Run a fresh 10-minute monitor soak with no 429, duplicate process, memory growth, login-profile loss, or checkout-guard failure. The final baseline started at 2026-08-05 17:59:59.948839 KST on installed main `a85ce26` and passed at 18:10:05 KST after 605 seconds: status remained `armed` with `match: null`; there was exactly one Prickly monitor and one Playwright driver; `rate_limited`, `monitor_error`, `configuration_invalid`, `login_required`, `checkout_pre_submit_error`, and `checkout_guard_retry_deferred` were all zero. RSS decreased from 51,296 to 39,232 KiB for the monitor, 171,984 to 75,168 KiB for the driver, and 630,208 to 570,128 KiB across the eight dedicated Chrome processes. The dedicated browser state remained present, the old `cgv-imax` Codex automation remained paused, and no process held `/Users/woojinyoung/.hermes/browser-profiles/cgv`. The first soak was invalidated on 2026-08-05 when a real seat match exposed a changed CGV mobile-ticket empty-state selector; it stopped before seat selection or payment. The 2026-08-05 16:35:02 KST baseline also failed safely after the dedicated CGV tab closed: the runtime stayed out of checkout with `match: null`, but recorded 21 `monitor_error` events and remained `recovering`; closed-tab/CDP recovery is covered by regression tests in the passing build.
...
- [x] Install and verify the minimum-lead/iPhone-email build `56bbee8` on macOS (2026-08-09). In a clean dependency environment the complete local suite passed 153 tests; Ruff 0.14.2, compileall, macOS shell syntax, and diff checks passed. Repository and installed files matched at SHA-256 `b69c87818a856beadd6bba7859645a95b96a8906e30c043e0f9e39fcc9738d40` for `policy.py`, `43e7a4eebbf69221ffd38961be9a24bffff08d29d6654e2fa469e4e09f05e2ef` for `scheduler.py`, and `4a4ef8a94b89c832977bdbf036b77fedd8f95368fb34dd1d30acea992c7701f4` for `config.py`. The preserved legacy local configuration resolved to `minimum_lead_minutes: 180`; `doctor` passed, the dedicated browser profile remained present with 1,709 files, and the final resident state was `armed` with `match: null`, exactly one monitor, exactly one Playwright driver, and zero processes using the Hermes CGV profile. No showtime, party, seat, voucher, or payment control was clicked during this verification. The selected result notifications still use desktop plus email; iPhone Mail can receive that email, but delivery delay, Focus, silent mode, and waking the user are not guaranteed.
- [x] Install and verify balanced hot-scan build `05067ed` on macOS (2026-08-14). The complete local suite passed 167 tests and hosted macOS/Windows checks passed in PR `#26`. The source and installed runtime matched at SHA-256 `84f60e0a592d4b8d93316d544f36dd8f44bfdf402d36611caf9598d7e94d7cb3` for `monitor.py` and `bedd7795fdc2cb129894a26cdbbcc0599ee944fa844d28f55b7ea1af9ad56181` for `scheduler.py`; the preserved configuration checksum remained unchanged and the dedicated Chrome profile retained 1,908 files. A fresh 10-minute baseline started at 2026-08-14 03:15:31.948521 KST and passed at 03:25:42 KST: status remained `armed` with `match: null`, exactly one monitor and one Playwright driver remained active, and `rate_limited`, `monitor_error`, `configuration_invalid`, `login_required`, `checkout_pre_submit_error`, and `checkout_guard_retry_deferred` were all zero. RSS changed from 48,832 to 57,424 KiB for the monitor, 166,800 to 169,792 KiB for the driver, and 1,097,408 to 1,241,024 KiB across seven dedicated Chrome processes, all below the material-growth threshold. A standalone process audit found no Hermes CGV-profile process. No showtime, party, seat, voucher, or payment control was clicked during installation or soak verification. The runtime used four hot seat-map scans per discovery scan; with 37 eligible shows its disclosed revisit estimate was 47 seconds under the one-request-per-second public-IP limit.
