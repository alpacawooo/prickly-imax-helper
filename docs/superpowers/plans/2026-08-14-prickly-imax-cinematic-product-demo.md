...
- [ ] **Step 3: Add the approved composition values to the manifest**
...
[
  ...
  [3, "historical-loop", "bottom", "staged-reveal"],
...
Card 3 uses redacted historical Prickly attempt-stage evidence already stored locally; it must not trigger a fresh CGV page load. Cards 4–7 use real offline local surfaces.

- [ ] **Step 4: Strengthen `validate_manifest()`**
...
- [ ] **Step 1: Write failing anti-template HTML tests**
...
- [ ] **Step 3: Implement shared typography and minimal chrome**
...
- [ ] **Step 4: Implement each approved card composition**
...
3. Redacted historical attempt stages as one large evidence field with uneven opacity and a bottom statement; no browser frame.
...
- [ ] **Step 5: Render covers and inspect the contact sheet**
...
**Interfaces:**
- Consumes: local setup HTML, redacted `prickly-imax diagnose`, redacted historical event stages, local guide HTML, cover PNGs, and manifest motion names.
- Produces: `redacted_monitor_preview() -> Path`, `render_motion_frames(card, cover) -> list[Path]`, and eight final MP4 files.

- [ ] **Step 1: Write failing privacy and motion tests**
...
- [ ] **Step 3: Add exact motion recipes**
...
- [ ] **Step 4: Capture real offline evidence**

Render the setup page and installation guide through the existing offline functions. Run the installed CLI's redacted diagnose command without opening Chrome or contacting CGV, pass its output through `redact_visual_evidence()`, and render it as plain actual output rather than invented dashboard tiles. Read existing local attempt-stage events only to produce Card 3's real sequence; include stage names and outcomes, not seat numbers, dates, attempt IDs, profile paths, or account data.

- [ ] **Step 5: Implement card-specific video assembly**
...
- [ ] **Step 6: Review animations with the installed motion skills**
...
- [ ] **Step 1: Write failing output-metadata tests**
...
- [ ] **Step 4: Run the complete build and automated tests**
...
- [ ] **Step 5: Perform human visual QA**
...
- [ ] **Step 1: Update the source and safety ledger**

Record the two approved Odyssey still sources, local setup renderer, redacted diagnose source, historical event-stage source, local guide renderer, excluded benchmark recordings, no-CGV-request production boundary, and the exact reason no fake browser/terminal/phone UI appears.

- [ ] **Step 2: Update operator instructions**
...
- [ ] **Step 4: Verify the package and repository diff**
