...
저장할 때 macOS는 Apple Mail, Windows는 클래식 Outlook 데스크톱으로 테스트 메일을 한 번 보냅니다. macOS가 Mail 제어 권한을 물으면 `허용`을 누릅니다. Windows에서는 Outlook에 미리 로그인해 두고 프로그램 액세스 경고가 나오면 내용을 확인한 뒤 허용합니다. 테스트 발송에 실패하면 설정은 저장되지 않습니다.
