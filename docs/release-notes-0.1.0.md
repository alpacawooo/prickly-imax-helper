...
이 prerelease는 공개 가능 테스트 식별자 `PUB-TEST-73464A55D011`의 허용 범위에 따라 사전에 승인된 제한 인원의 비공개 파일럿에만 배포한다. 일반 공개·재배포·암표 목적 사용은 허용하지 않는다. 24시간 안정성 검사와 실제 비개발자 3명 파일럿이 끝나기 전에는 베타 성공이나 정식 릴리스로 표시하지 않는다.
